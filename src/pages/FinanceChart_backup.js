...
// financeData 캐시: key → { weekly: [{date, close_krw, close_usd}] }
...
// ────────────────────────────────────────────
function PriceChart({ selected, currency, yearWindow, isMobile }) {
  ...
  useEffect(() => {
    ...
    const dataStartDate = yearsAgo(yearWindow + 100);
    ...
    Promise.allSettled(keys.map(k => fetchFinanceData(k)))
      .then((results) => {
        ...
        const seriesRefs = [series1Ref, series2Ref, series3Ref];
        results.forEach((result, idx) => {
          if (result.status === 'rejected') return;
          const { weekly } = result.value;
          const points = filterByDate(weekly, dataStartDate, currency);
          if (!points.length) return;

          const assetInfo = ASSET_MAP[keys[idx]];
          const isRate = !!assetInfo.isRate;
          const scaleId = mixedTypes
            ? (isRate ? 'right' : 'left')
            : 'right'; // 단일 타입이면 모두 같은 Y축
...
// ────────────────────────────────────────────
function NormChart({ selected, currency, yearWindow, normMonthsAgo = 36, isMobile, aptX = [], aptAvg = [], aptName = null, showApt = true }) {
  const containerRef = useRef(null);
  const chartRef     = useRef(null);
  ...
  useEffect(() => {
    ...
    chart.timeScale().subscribeVisibleTimeRangeChange(() => {
      if (baseTimeRef.current) {
        const x = chart.timeScale().timeToCoordinate(baseTimeRef.current);
        setBaseLineX(x != null ? x : null);
      }
    });
  ...
  useEffect(() => {
    ...
    const visibleStartDate = yearsAgo(Math.max(yearWindow, normYears + 1));
    const effectiveStartDate = yearsAgo(100); // 전체 데이터 로드 (스크롤 허용)
    ...
    Promise.allSettled(selected.map(k => fetchFinanceData(k)))
      .then((results) => {
        ...
        results.forEach((result, idx) => {
          if (result.status === 'rejected') return;
          const { weekly } = result.value;
          const raw = filterByDate(weekly, effectiveStartDate, currency);
          if (!raw.length) return;

          const a = ASSET_MAP[selected[idx]];

          let points;
          if (a?.isRate) {
            // 금리 자산: 전체 기간 최대값 = 100% 고정
            const maxValue = Math.max(...raw.map(p => p.value));
            if (!maxValue) return;
            points = raw.map(p => ({ ...p, value: Math.round((p.value / maxValue) * 10000) / 100 }));
            // 기준선은 첫 번째 비율 자산이 없을 때만 fallback
            if (idx === 0 && !baseTimeRef.current) {
              const baseTs = new Date(baseDateStr).getTime();
              const basePoint = raw.reduce((prev, curr) =>
                Math.abs(new Date(curr.time).getTime() - baseTs) <
                Math.abs(new Date(prev.time).getTime() - baseTs) ? curr : prev
              );
              baseTimeRef.current = basePoint.time;
            }
          } else {
            // 일반 자산: 기준 날짜 = 100%
            const baseTs = new Date(baseDateStr).getTime();
        ...
        const today = new Date().toISOString().slice(0, 10);
        requestAnimationFrame(() => {
          if (chartRef.current && seriesRefs.current.length > 0) {
            try {
  ...
  return (
    <div style={{ position: 'relative', width: '100%' }}>
      ...
      {/* 범례 */}
      <div style={{ display: 'flex', gap: 10, padding: '4px 4px 6px', flexWrap: 'wrap' }}>
        {selected.map(k => {
          const a = ASSET_MAP[k];
          return (
        ...
        {aptName && aptAvg.length > 0 && (
          <span style={{ display: 'flex', alignItems: 'center', gap: 5, fontSize: '0.78rem', fontWeight: 700, color: '#1F1D1B' }}>
            <span style={{ width: 16, height: 3, background: 'rgba(255, 127, 14, 0.7)', display: 'inline-block', borderRadius: 2 }} />
            {aptName}
          </span>
        )}
      </div>
...
export default function FinanceChart({ isMobile = false, aptX = [], aptAvg = [], aptName = null, onOpenChartPanel }) {
  ...
  const dataEarliestDate = selected
    .map(k => financeCache.get(k)?.weekly?.[0]?.date)
    .filter(Boolean)
    .sort()
    .at(-1);
  ...
  return (
    <div style={{ marginTop: 0, padding: '0 0 4px' }}>
              ...
              {aptName && aptAvg.length > 0 && (
                ...
                >
                  ...
                  <span style={{
                    ...
                    color: showApt ? 'rgba(196,154,42,0.9)' : '#888780',
                  }}>
                    {aptName}
                  </span>
                </div>
      ...
      {(() => {
        const rateKeys  = selected.filter(k => ASSET_MAP[k]?.isRate);
        const priceKeys = selected.filter(k => !ASSET_MAP[k]?.isRate);
        return (
          <>
            {rateKeys.length > 0 && (
              <div style={{ borderTop: '1px solid #E6DED4', paddingTop: 8, marginBottom: 4 }}>
                <div style={{ fontSize: '0.92rem', fontWeight: 800, marginBottom: 0 }}>
                  {rateKeys.map((k, i) => (
                    <span key={k}>
            ...
            {priceKeys.map(k => {
              ...
              return (
                <div key={k} style={{ borderTop: '1px solid #E6DED4', paddingTop: 8, marginBottom: 4 }}>
                  ...
                  <PriceChart selected={[k]} currency={currency} yearWindow={yearWindow} isMobile={isMobile} />
                </div>
              );
            })}
          </>
