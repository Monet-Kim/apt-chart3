...
export function buildAptInfoPairs(listRow, detailRow, { includeAddress = true } = {}) {
  ...
  const pairs = [
    ...
    ['관리직원수',   (() => { const cnt = parseFloat(D('kaptMgrCnt') || ''); if (!Number.isFinite(cnt)) return null; const units = parseFloat(L('kaptdaCnt') || ''); const per = (Number.isFinite(units) && units > 0) ? (cnt / units).toFixed(2).replace(/\.?0+$/, '') : null; return per ? `${cnt}명 (세대당 ${per}명)` : `${cnt}명`; })()],
    ['관리회사',     D('kaptCcompany')],
    ['경비원수',     (() => { const cnt = parseFloat(D('kaptdScnt') || ''); if (!Number.isFinite(cnt)) return null; const units = parseFloat(L('kaptdaCnt') || ''); const per = (Number.isFinite(units) && units > 0) ? (cnt / units).toFixed(2).replace(/\.?0+$/, '') : null; return per ? `${cnt}명 (세대당 ${per}명)` : `${cnt}명`; })()],
    ['경비용역사',   D('kaptdSecCom')],
    ['청소부수',     (() => { const cnt = parseFloat(D('kaptdClcnt') || ''); if (!Number.isFinite(cnt)) return null; const units = parseFloat(L('kaptdaCnt') || ''); const per = (Number.isFinite(units) && units > 0) ? (cnt / units).toFixed(2).replace(/\.?0+$/, '') : null; return per ? `${cnt}명 (세대당 ${per}명)` : `${cnt}명`; })()],
    ['전기용량(kW)', 전기용량],
    ['화재경보기',   D('codeFalarm')],
